	...
	: ShaderBase(dev, L"../Assets/Shaders/TextureLight.hlsl")
{
	...
	// Color buffer
	bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(Data_WorldAndSpec);
	bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	bd.CPUAccessFlags = 0;
	bd.MiscFlags = 0;
	bd.StructureByteStride = 0;
	hr = this->GetDevice()->CreateBuffer(&bd, nullptr, &mpBuffWordAndSpec);
	assert(SUCCEEDED(hr));
...
ShaderTextureLight::~ShaderTextureLight()
{
	...
	ReleaseAndDeleteCOMobject(mpBuffWordAndSpec);
...
void ShaderTextureLight::SetPointLightParameters(PointLight_ToShare PLIn[])
{
	for (int i = 0; i < PLCount; i++)
	{
		PointLight_ToShare LightToAdd = PLIn[i];
		PointLightData[i].Light.Ambient = LightToAdd.GetAmbient();
		PointLightData[i].Light.Diffuse = LightToAdd.GetDiffuse();
		PointLightData[i].Light.Specular = Vect(1, 1, 1);
		PointLightData[i].Position = LightToAdd.GetPosition();
		PointLightData[i].Attenuation = LightToAdd.GetAttenuation();
		PointLightData[i].Range = LightToAdd.GetRange();
	}

}
void ShaderTextureLight::SetPointLightPos(const Vect& pos, int i)
{
	PointLightData[i].Position = pos;
}
void ShaderTextureLight::SetPointLightParameters(int i, const Vect& pos, float r, const Vect& att, const Vect& amb, const Vect& dif, const Vect& sp)
{
	PointLightData[i].Light.Ambient = amb;
	PointLightData[i].Light.Diffuse = dif;
	PointLightData[i].Light.Specular = sp;
	PointLightData[i].Position = pos;
	PointLightData[i].Attenuation = att;
	PointLightData[i].Range = r;
}

void ShaderTextureLight::SetSpotLightPos(const Vect& pos)
{
...
void ShaderTextureLight::SendWorldAndSpec(const Matrix& world, const Vect& sp)
{
	Data_WorldAndSpec wm;
	wm.World = world;
	wm.WorlInv = world.getInv();
	wm.Mat.Specular = sp;

	this->GetContext()->UpdateSubresource(mpBuffWordAndSpec, 0, nullptr, &wm, 0, 0);
}
...
void ShaderTextureLight::SetToContext(ID3D11DeviceContext* devcon)
{
	...
	devcon->VSSetConstantBuffers(2, 1, &mpBuffWordAndSpec);

	devcon->PSSetConstantBuffers(0, 1, &mpBufferCamMatrices);
	devcon->PSSetConstantBuffers(1, 1, &mpBufferLightParams);
	devcon->PSSetConstantBuffers(2, 1, &mpBuffWordAndSpec);
	devcon->PSSetConstantBuffers(3, 1, &mpBufferFogParams);
}

void ShaderTextureLight::SetFogData(float FogStartIn, float FogRangeIn, Vect &FogColorIn)
{
	FogData fD;
