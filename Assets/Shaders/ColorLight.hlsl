...
float4 PS(VS_OUTPUT input) : SV_Target
{
	...
	ComputePointLight(Mater, PntLight01, input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);

	ambient += A;
	diffuse += D;
	spec += S;

	ComputePointLight(Mater, PntLight02, input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);

	ambient += A;
	diffuse += D;
	spec += S;

	ComputePointLight(Mater, PntLight03, input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);

	ambient += A;
	diffuse += D;
	spec += S;

	ComputePointLight(Mater, PntLight04, input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);
	...
	if (input.WorldPos.y < 5 && input.WorldPos.y >= 0)
	{
		ColorPercent = input.WorldPos.y / 5;
		ColorPercent = 1 - ColorPercent;
		ColorPercent *= FogColor.w;
	}

	else if(input.WorldPos.y < 0)
	{
		float Depth = input.PosMS.y - 5;
		ColorPercent = Depth / 5;
		//ColorPercent = 1 + ColorPercent;
		//ColorPercent *= FogColor.w;
		//ColorPercent = 1.0f;
		//ColorPercent = 1.0f;
